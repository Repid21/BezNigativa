...
function UntitledBoxingGame:ResolveEffectHelper()
    ...
    if not moduleScript then
        self:AddDiagnostic("Auto Dodge initialization failed.\n" .. tostring(getGcError), true)
    ...
    if type(getrenv) == "function" then
        ...
        if type(runtimeRequire) == "function" and runtimeRequire ~= require then
            ...
            if runtimeOk and isEffectHelper(runtimeResult) then
                ...
                self:AddDiagnostic("EffectHelper API найден через getrenv().require.", false)
                return runtimeResult, self.EffectHelperSource
            end
            if not runtimeOk then
                self:AddDiagnostic("getrenv().require also crashed.\nModule: " .. fullName(moduleScript) .. "\nFull error/dependency traceback:\n" .. tostring(runtimeResult), true)
            end
        end
    end
    ...
    if loaded then
        ...
        self:AddDiagnostic("EffectHelper API появился в getgc после require.", false)
        return loaded, self.EffectHelperSource
    end
    self:AddDiagnostic("Auto Dodge initialization failed after every resolver.\nModule: " .. fullName(moduleScript)
        .. "\ngetgc diagnostic: " .. tostring(getGcError), true)
...
function UntitledBoxingGame:InstallHooks()
    ...
    if not helper then
        self:AddDiagnostic("Auto Dodge is not Ready: " .. tostring(sourceOrError), true)
