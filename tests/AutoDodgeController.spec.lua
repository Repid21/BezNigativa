...
test("9 - M1 to M2 replacement is atomic", function()
    local controller, state, advance = harness()
    controller:Start("enemy", 901, "M1", 0, 0.8)
    controller:Commit("enemy", 901, "M1", 0.8)
    advance(0.1)
    controller:Start("enemy", 902, "M2", 0.1, 1)
    controller:Commit("enemy", 902, "M2", 1)
    advance(1)
    expectEqual(state.dodges, 1, "atomic replacement must only dodge M2")
    expectEqual(controller.Handled[901], nil, "replaced M1 must not execute")
    expectEqual(controller.Handled[902], true, "replacement M2 must execute")
end)

test("10 - M2 to M1 replacement is atomic", function()
    local controller, state, advance = harness()
    controller:Start("enemy", 1001, "M2", 0, 0.8)
    controller:Commit("enemy", 1001, "M2", 0.8)
    advance(0.1)
    controller:Start("enemy", 1002, "M1", 0.1, 1)
    controller:Commit("enemy", 1002, "M1", 1)
    advance(1)
    expectEqual(state.dodges, 1, "atomic replacement must only dodge M1")
    expectEqual(controller.Handled[1001], nil, "replaced M2 must not execute")
    expectEqual(controller.Handled[1002], true, "replacement M1 must execute")
end)

print("AutoDodgeController: " .. tostring(passed) .. "/10 acceptance tests passed")
