...
# Services + safe test environment
if 'local HttpService = game:GetService("HttpService")' not in s:
    s = s.replace(
...
if "local safeEnvironment = RunService:IsStudio()" not in s:
    marker = '''if not LocalPlayer then
    warn("[BezNigativa] LocalPlayer not found")
    return
end

local env = (getgenv and getgenv()) or _G'''
    replacement = '''if not LocalPlayer then
    warn("[BezNigativa] LocalPlayer not found")
    return
end
...
local env = (getgenv and getgenv()) or _G'''
    if marker not in s:
        raise SystemExit("safeEnvironment insertion point not found")
...
# Generic config-save hook for UI controls
if "local requestConfigSave = nil" not in s:
    s = s.replace(
...
old_toggle = '''    bind(button.MouseButton1Click:Connect(function()
        state = not state
        if callback then
            callback(state)
        end
...
new_toggle = '''    bind(button.MouseButton1Click:Connect(function()
...
if old_toggle not in s:
    raise SystemExit("toggle click block not found")
s = s.replace(old_toggle, new_toggle, 1)

# Replace +/- steppers with draggable sliders + editable numeric input.
start = s.find("local function createStepper(")
finish = s.find("\nlocal drawingSupported = false", start)
if start < 0 or finish < 0:
    raise SystemExit("createStepper region not found")

slider_code = r'''local function createStepper(parent, y, labelText, initial, minimum, maximum, step, onChanged)
    local value = initial
...
'''
s = s[:start] + slider_code + s[finish:]

# Restore test-only gate to Movement and Combat.
s = s.replace(
...
def add_gate(first_line: str, state_line: str):
    global s
    marker = first_line + "\n    " + state_line
    replacement = first_line + "\n    if value and not safeEnvironment then return false end\n    " + state_line
    if marker not in s:
        raise SystemExit("toggle gate point not found: " + first_line)
    s = s.replace(marker, replacement, 1)

add_gate('local speedToggle = createToggle(MovementPage, 18, 78, 190, "Speed", false, function(value)', 'speedEnabled = value')
add_gate('local jumpToggle = createToggle(MovementPage, 222, 78, 190, "Jump", false, function(value)', 'jumpEnabled = value')
add_gate('local noclipToggle = createToggle(MovementPage, 18, 122, 190, "NoClip", false, function(value)', 'noclipEnabled = value')
add_gate('local flyToggle = createToggle(MovementPage, 222, 122, 190, "Fly", false, function(value)', 'flyEnabled = value')
add_gate('local cameraToggle = createToggle(CombatPage, 18, 78, 190, "AimBot", false, function(value)', 'cameraAssistEnabled = value')
...
# Numeric controls: slider drag snaps to step; text input accepts exact number.
s = s.replace(
...
# Aim settings: wall check + actual convergence speed.
if "local wallCheckEnabled" not in s:
    s = s.replace("local smoothness = 8", "local aimSpeed = 35\nlocal wallCheckEnabled = true", 1)
...
wall_marker = '''local getAimSpeed, setAimSpeed = createStepper(CombatPage, 172, "Aim speed", aimSpeed, 1, 100, 1, function(v) aimSpeed = v end)

local groupsLabel = Instance.new("TextLabel")'''
wall_replacement = '''local getAimSpeed, setAimSpeed = createStepper(CombatPage, 172, "Aim speed", aimSpeed, 1, 100, 1, function(v) aimSpeed = v end)
...
local groupsLabel = Instance.new("TextLabel")'''
if wall_marker not in s:
    raise SystemExit("wall check UI insertion point not found")
s = s.replace(wall_marker, wall_replacement, 1)

los_marker = '''local function getBestAimPoint()
    if not Camera then return nil end'''
los_replacement = '''local function hasLineOfSight(targetCharacter, worldPosition)
...
local function getBestAimPoint()
    if not Camera then return nil end'''
if los_marker not in s:
    raise SystemExit("line of sight insertion point not found")
s = s.replace(los_marker, los_replacement, 1)

visibility = "if visible and screenPosition.Z > 0 then"
if visibility not in s:
    raise SystemExit("target visibility condition not found")
s = s.replace(
    visibility,
...
old_smooth = '''    local desired = CFrame.lookAt(Camera.CFrame.Position, targetPosition)
    local alpha = math.clamp(1 / math.max(smoothness, 1), 0.02, 1)
    Camera.CFrame = Camera.CFrame:Lerp(desired, alpha)'''
new_smooth = '''    local desired = CFrame.lookAt(Camera.CFrame.Position, targetPosition)
...
if old_smooth not in s:
    raise SystemExit("aim speed block not found")
s = s.replace(old_smooth, new_smooth, 1)

old_bind = '''RunService:BindToRenderStep(AIMBOT_RENDER_NAME, Enum.RenderPriority.Camera.Value + 1, function()
    Camera = workspace.CurrentCamera or Camera
    updateCombat()
end)'''
new_bind = '''RunService:BindToRenderStep(AIMBOT_RENDER_NAME, Enum.RenderPriority.Camera.Value + 1, function(deltaTime)
    Camera = workspace.CurrentCamera or Camera
    updateCombat(deltaTime)
end)'''
if old_bind not in s:
    raise SystemExit("aim render binding not found")
s = s.replace(old_bind, new_bind, 1)

# Config: Xeno/executor file API when available. Auto-load + debounced autosave + manual buttons.
other_marker = '''-- OTHER
pageTitle(OtherPage, "Other", "No modules yet")'''
config_code = r'''-- OTHER / CONFIG
...
if other_marker not in s:
    raise SystemExit("Other page marker not found")
s = s.replace(other_marker, config_code, 1)
